...
const elemento = document.querySelector("#createdBy")
const contenidoHTML = elemento.innerHTML;
elemento.innerHTML = contenidoHTML + "Osvaldo"
...
function buildToDo(todo, index) {
  // Tu código acá:
   let objeto = new ToDo(todo);
   let indice = index;
   const toDoShell = document.createElement('div');
   toDoShell.classList.add('toDoShell');
   const toDoText = document.createElement('span');

   const contenidoHTML = toDoText.innerHTML;
    toDoText.innerHTML = objeto.description; 
    toDoText.classList.add(indice);
    if(objeto.complete){
...
function buildToDos(toDos) {
  // Tu código acá:
  let newArreglo = toDos.map(buildToDo);
...
//  1) Seleccionr el elemento cuyo id es 'toDoContainer' y almacenarlo en una variable 
// denominada 'toDoContainer'
//  2) Setear el innerHTML de 'toDoContainer' como un string vacio ("")
//  3) Llamar a la función previemante creada 'buildToDos' pasándole como argumento el 
// array toDoItems
//  4) Iterar sobre el resultado devuelto por la función 'buildToDos' e ir agregndo cada 
// elemento a 'toDoContainer'
//  5) Al final de este archivo, antes de la línea que dice "NO CAMBIES NADA DE ACÁ PARA 
// ABAJO" escribe una
...
// [NOTA: Algunas cuestiones a tener en cuenta sobre el elemento 'input' de HTML 
//(Ya que 'toDoInput' es un input)
// Todos los elementos input tienen una propiedad llamada 'value' que nos permite 
//acceder al texto que se encuentre
// actualmente escrito dentro del input]
//  1) Crear un nuevo ToDo usando la clase ToDo y pasándole el valor del input 
//'toDoInput' como parámetro
//  2) Agregar el objeto ToDo recién creado al array toDoItems
//  3) Setear el valor del input toDoInput como un string vacio ("") 
//(Esto se realiza para que en la vista se borre lo que se encontraba escrito)
//  4) Llamar a la función displayToDos para que se actualicen los toDos 
//mostrados en pantalla

function addToDo() {
  // Tu código acá:
  let ingresar = new ToDo(toDoInput.value);
  toDoItems.push(ingresar.description);
  toDoInput.innerText = "";
  displayToDos();
}
...
// Todo Event Listener recibe como parámetro el objeto 'event' conteniendo un montón de
//  información que incluye
// el tipo del evento, que elemento fue el que lo llamó, los valores de dicho elemento, etc.
// En este paso vamos a utilizarlo para encontrar el index del item que disparó el evento 
// (Esta parte ya se
// encuentra desarrollada pero está comentada dentro de la función por lo que va a ser 
// necesario que la descomenten)]
//   1) Utilizando el index suministrdo, llamar a 'completeToDo' (Recuerden que habíamos 
// creado dcho método en el
...
//   3) En la función 'buildToDo' agregar un 'click' event listener al elemento 'toDoText', 
// pasándole
